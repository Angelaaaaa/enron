...
class ReadFile:
    ...
    def readNames(self,path):
        ...
        for file in files:  # 遍历文件夹
            ...
            if not os.path.isdir(file):
                ...
                if os.path.isdir(path+"/"+file+"/sent"):
                    files = os.listdir(path+"/"+file+"/sent")
                    print(".")
                    self.fileProcess(file, files, path, emailDict, subPath="sent")
                else:
                    if os.path.isdir(path+"/"+file+"/sent_items"):
                        files = os.listdir(path+"/"+file+"/sent_items")
                        print(".")
                        self.fileProcess(file, files, path, emailDict, subPath="sent_items")

                    else:
                        if os.path.isdir(path + "/" + file + "/_sent_mail"):
                            files = os.listdir(path + "/" + file + "/_sent_mail")
                            print(".")
                            self.fileProcess(file,files,path,emailDict,subPath="_sent_mail")
    ...
    def fileProcess(self,file,files,path,emailDict,subPath):
        ...
        for fileNum in files:  # open sent files
            if not os.path.isdir(fileNum) and fileNum != ".DS_Store":  # 判断是否是文件夹，不是文件夹才打开
    ...
    def findReceiver(self,file,files,path,sendDict,subPath):
        ...
        for fileNum in files:  # open sent files
            if not os.path.isdir(
                    fileNum) and fileNum != ".DS_Store":  # 判断是否是文件夹，不是文件夹才打开
                # f = open(path + "/"+file+"/sent/" + fileNum)# 打开文件
                with open(path + "/" + file + "/" + subPath + "/" + fileNum,
                          errors='ignore') as f:
                    for line in f.readlines():  # find sender
        ...
        for file in files:  # 遍历文件夹
            ...
            if not os.path.isdir(file):
                ...
                if os.path.isdir(path + "/" + file + "/sent"):
                    files = os.listdir(path + "/" + file + "/sent")
                    print(".")
                    self.findReceiver(file, files, path, sentDict, subPath="sent")
                else:
                    if os.path.isdir(path + "/" + file + "/sent_items"):
                        files = os.listdir(path + "/" + file + "/sent_items")
                        print(".")
                        self.findReceiver(file, files, path, sentDict, subPath="sent_items")

                    else:
                        if os.path.isdir(path + "/" + file + "/_sent_mail"):
                            files = os.listdir(path + "/" + file + "/_sent_mail")
                            print(".")
                            self.findReceiver(file, files, path, sentDict, subPath="_sent_mail")
